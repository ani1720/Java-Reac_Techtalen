...
public class Tarea03UD06App {
	...
	public static void main(String[] args) {
		...
		// usamos un bucle if donde le decimos que si el numero inrresado es menor o igual 1 
		if (numPrimo <= 1) {
			JOptionPane.showMessageDialog(null, String.format("El %d es primo es <=1", numPrimo));
			//decimos que si no es menor o igual a 1. Entonces es un numero primo
		}  else if (esPrimo(numPrimo)) {
	...
	public static boolean esPrimo(int valorPrimo) {
		// Verificamos mediante un bucle SI el valor es menor o igual a 1 nos devuelve FALSE
		if (valorPrimo <=1) {
			return false;
		}
/* Utilizamos el bucle for para comprobar si el numero tiene divisores distintos
 * a 1 y de el mismo. Entonces recorrera desde 2 hasta la raiz cuadrada del numero
 * Asi que si encuentra divisor donde de 0 devuelve FALSE y sino es munero primo y devuelve TRUE
**/
